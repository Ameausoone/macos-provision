...
function push() {
  ...
  if [[ -z "${commit_msg}" ]]; then
    commit_msg=$(copilot --allow-tool 'shell(git:*)' --prompt "Write a commit msg from the git diff, #conventional commit, output exclusively the result starting with feat|fix|chose.. without any comment" --silent)
    if [[ -z "${commit_msg}" ]]; then
